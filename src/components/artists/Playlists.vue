  ...
  <div class="relative pt-12 pb-24">
    <ul class="w-[90vw] md:w-[70vw] mx-auto grid md:grid-cols-2 gap-x-[20vw]">
      <li v-for="playlist in playlistItems" :key="playlist.name">
        <div @click="setActivePlaylist(playlist)" class="flex gap-8 hovercrimson cursor-pointer text-xl md:text-sm">
    ...
    <div class="pt-10 w-[50vw] mx-auto">
      <a :href="activePlaylist.link">
        <img :src="activePlaylist.image" class="w-full h-full object-contain" alt="playslist image">
      </a>
    </div>
  </div>
</section>
